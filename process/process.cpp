...
std::array<DWORD, 2> Process::GetModuleBaseAddress(const char* modName)
{
    PDWORD32 modBaseAddr = 0;
    ...
    if (hSnap != INVALID_HANDLE_VALUE)
    {
        ...
        if (Module32First(hSnap, &modEntry))
        {
            do
            {
                ...
                {
                    modBaseAddr = (PDWORD32)modEntry.modBaseAddr;
                    break;
                }
    ...
    CloseHandle(hSnap);
    return { reinterpret_cast<DWORD>(modBaseAddr), modEntry.modBaseSize };
}